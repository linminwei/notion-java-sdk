...
import com.minwei.anno.NotionConfig;
...
public class NotionUtil {
    ...
    public static <T> NotionConfigDTO getNotionConfig(Class<T> clazz) {
        // 通过类对象获取到Object实例
        T object;
        try {
        ...
        return getNotionConfigDTO(clazz, object);
    }
     ...
     */
    public static <T> NotionConfigDTO getNotionConfig(T object) {
        // 获取Object类对象
        Class<?> clazz = object.getClass();
        return getNotionConfigDTO(clazz, object);
    }

    private static <T> NotionConfigDTO getNotionConfigDTO(Class<?> clazz, T object) {
        // 获取配置在类上的注解NotionConfig
        NotionConfig notionConfig = clazz.getAnnotation(NotionConfig.class);
        if (notionConfig == null) {
            throw new RuntimeException("该Notion业务实体未配置NotionConfig");
        }
        // 获取数据库id
        String databaseId = notionConfig.databaseId();
        // 获取token
        String token = notionConfig.token();
        // 获取对象的所有字段
        Field[] fields = clazz.getDeclaredFields();
        List<NotionConfigDTO.Property> properties = Arrays.stream(fields).map(field -> {
            // 暴力反射
            ...
            return NotionConfigDTO.Property.builder()
                    .name(name)
                    .type(type)
                    .value(value)
                    .isIcon(icon)
        ...
        return NotionConfigDTO.builder()
     ...
     */
    public static Map<String, PageProperty> buildProperties(List<NotionConfigDTO.Property> properties) {
     ...
     * @return File
     */
    public static File getFileFromConfig(List<NotionConfigDTO.Property> properties, Predicate<NotionConfigDTO.Property> predicate) {
        List<NotionConfigDTO.Property> filteredConfigs = properties.stream()
                .filter(predicate)
                .collect(Collectors.toList());

        if (!filteredConfigs.isEmpty()) {
            NotionConfigDTO.Property propertyConfig = filteredConfigs.get(0);
    ...
    private static RelationFilter getRelationFiler(String filterType, String relationId) {
        if (FilterTypeConstant.CONTAINS.equals(filterType)) {
            return new RelationFilter(relationId);
        }
        if (FilterTypeConstant.NOT_CONTAINS.equals(filterType)) {
            return new RelationFilter(null, relationId);
        }
        if (FilterTypeConstant.IS_EMPTY.equals(filterType)) {
            return new RelationFilter(null, null, true);
        }
        if (FilterTypeConstant.IS_NOT_EMPTY.equals(filterType)) {
    ...
    private static CheckboxFilter getCheckboxFilter(String filterType) {
        if (FilterTypeConstant.EQUAL.equals(filterType)) {
            return new CheckboxFilter(true);
        }
        if (FilterTypeConstant.NOT_EQUAL.equals(filterType)) {
    ...
    private static NumberFilter getNumberFilter(String filterType, Integer filterValue) {
        ...
        if (FilterTypeConstant.GREATER_THAN.equals(filterType)) {
            return new NumberFilter(null, null, filterValue);
        }
        if (FilterTypeConstant.LESS_THAN.equals(filterType)) {
            return new NumberFilter(null, null, null, filterValue);
        }
        if (FilterTypeConstant.GREATER_THAN_OR_EQUAL_TO.equals(filterType)) {
            return new NumberFilter(null, null, null, null, filterValue);
        }
        if (FilterTypeConstant.LESS_THAN_OR_EQUAL_TO.equals(filterType)) {
    ...
    private static TextFilter getTextFilter(String filterType, String filterValue) {
        ...
        if (FilterTypeConstant.STARTS_WITH.equals(filterType)) {
            return new TextFilter(null, null, null, null, filterValue);
        }
        if (FilterTypeConstant.ENDS_WITH.equals(filterType)) {
