...
public class PageServiceImpl implements PageService {
    ...
    public <T> String createPage(T object) {
        ...
        // 获取icon配置
        File iconFile = NotionUtil.getFileFromConfig(config.getProperties(), NotionConfigDTO.Property::getIsIcon);
        // 获取cover配置
        File coverFile = NotionUtil.getFileFromConfig(config.getProperties(), NotionConfigDTO.Property::getIsCover);
        return client.createPage(PageParent.database(config.getDatabaseId()), pagePropertyMap, null, iconFile, coverFile).getId();
    }

    @Override
    public <T> List<String> batchCreatePage(List<T> objects) {
        return objects.stream().map(this::createPage).collect(Collectors.toList());
    }
